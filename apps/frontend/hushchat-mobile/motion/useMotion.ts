...
const EMPTY_PRESET: IMotionPreset = {
  initial: { opacity: 1, translateY: 0, scale: 1 },
 ...
 * - number
 * - { enter, exit }
 * - undefined → DS default 300/250
 * --------------------------------------------------------- */
const normalizeDuration = (
  duration: IMotionOptions["duration"],
  visible: boolean
) => {
...
/* ---------------------------------------------------------
 * 3. Easing normalizer (integrates MotionEasing fully)
 * Supports:
 * - easing="standard"
 * - easing="snappy"
 * - easing={MotionEasing.standard}
 ...
 * Priority:
 * from/to > initial/animate > preset > EMPTY_PRESET
 * --------------------------------------------------------- */
const mergeMotion = (
  ...
  const from: Partial<IMotionState> =
    opts.from ?? opts.initial ?? preset.initial;

  const to: Partial<IMotionState> =
    opts.to ?? opts.animate ?? preset.animate;
...
export const useMotion = (visible: boolean, opts: IMotionOptions = {}) => {
  ...
  const final = useMemo(
    () => mergeMotion(preset, opts, visible),
    [preset, opts, visible]
  );
  ...
  useEffect(() => {
    ...
    opacity.value = withDelay(
      delay,
      withTiming(target.opacity, { duration, easing })
    );

    translateY.value = withDelay(
      delay,
      withTiming(target.translateY, { duration, easing })
    );

    scale.value = withDelay(
      delay,
      withTiming(target.scale, { duration, easing })
    );
  }, [visible]);
